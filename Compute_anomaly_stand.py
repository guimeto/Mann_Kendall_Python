...
Code pour calculer les anomalies standardisées des indices
@author: guillaume
"""
import pandas as pd
import pathlib
...
yearmin = 1990                                                       #
yearmax = 2019                                                          #                                                                                                    
varin = 'Tasmax'                    # Tasmax, Tasmin, Tasmoy      
indice = 'MOY' 
...
for month in range(1,13,1):    
    for  index, row in list_nom.iterrows():
        ...
        modDfObj = data.apply(lambda x: (x - data_clim.mean()) / data_clim.std()  , axis=1)
        
        mypath_month='K:/DATA/Donnees_Stations/2nd_generation_V2019/INDICES/INDICES_MONTH/ANOMALY_STD/'+varin+'/'+str(indice)+'/'+str(yearmin)+'_'+str(yearmax)+'/'   
...
for season in ['DJF','JJA','MAM','SON']:       
    for  index, row in list_nom.iterrows():
        ...
        modDfObj = data.apply(lambda x: (x - data_clim.mean()) / data_clim.std()  , axis=1)
        modDfObj
        mypath_s='K:/DATA/Donnees_Stations/2nd_generation_V2019/INDICES/INDICES_SEASON/ANOMALY_STD/'+varin+'/'+str(indice)+'/'+str(yearmin)+'_'+str(yearmax)+'/'   
...
for  index, row in list_nom.iterrows():
    ...
    modDfObj = data.apply(lambda x: (x - data_clim.mean()) / data_clim.std()  , axis=1)
    mypath_y='K:/DATA/Donnees_Stations/2nd_generation_V2019/INDICES/INDICES_YEAR/ANOMALY_STD/'+varin+'/'+str(indice)+'/'+str(yearmin)+'_'+str(yearmax)+'/'   
